...
- [ ] **RULE-02**: 首批 P0 规则覆盖制造输出、BOM/CPL 和 manufacturer profile gate。
- [ ] **RULE-03**: 每条规则必须包含 ID、severity、适用对象、证据要求、风险说明和建议。
- [ ] **RULE-04**: 规则执行结果可以合并 KiCad 工具输出和自定义检查结果。
- [ ] **RULE-05**: 深电路语义规则首版降级为 `needs-human-review` checklist，不作为 blocking 自动结论。
...
- [ ] **RPT-04**: 报告包含项目概览、制造阻塞项、BOM/CPL 装配风险、KiCad ERC/DRC 解释、人工复核清单和证据附录。
